...
class Validator{
  ...
  public static function password($pwd){
    ...
    if( ctype_alnum($pwd) ){
      self::$errors['characters'] = 'must contain special characters';
    }
    ...
    if( $is_num == false ){
      self::$errors['number'] = 'must contain a number';
    }
    if( $is_upper == false ){
      self::$errors['uppercase'] = 'must contain an uppercase character';
    }
